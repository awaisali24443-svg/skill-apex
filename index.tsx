...
interface AppState {
    currentModule: any; // Can be more specific if module structure is known
    context: {
...
interface Route {
    ...
    module: string;
    name: string;
    // FIX: Made 'icon' property optional as not all routes have it.
    icon?: string;
    nav: boolean;
    footer?: boolean;
    fullBleed?: boolean;
 ...
 */
async function fetchModule(moduleName: string): Promise<any> {
...
function matchRoute(path: string): Route | null {
    for (const route of ROUTES) {
        ...
        const regexPath = route.path.replace(/:(\w+)/g, (_, paramName: string) => {
 ...
 */
async function loadModule(route: Route): Promise<void> {
    ...
    try {
        ...
        (document.getElementById('app-container') as HTMLElement).scrollTop = 0;
 ...
 */
function applyAppSettings(config: any): void {
...
async function main(): Promise<void> {
    try {
        ...
        renderSidebar(document.getElementById('sidebar') as HTMLElement);
        ...
        document.body.addEventListener('click', (event: MouseEvent) => {
            ...
            if ((event.target as HTMLElement).closest('.btn, .sidebar-link, .topic-button, .option-btn, .flashcard, .topic-card')) {
