    ...
    return `
        <a href="#${route.path}" class="sidebar-link ${extraClass}" data-path="${route.path}" aria-label="${route.name}">
            <div class="link-icon-wrapper">
                <svg class="icon"><use href="#${route.icon}"/></svg>
            </div>
...
export function renderSidebar(container) {
    ...
    const html = `
        <div class="sidebar-inner">
            <div class="sidebar-profile-header">
                ...
                <svg class="icon profile-chevron"><use href="#chevron-down"/></svg>
            </div>
            <div class="sidebar-divider"></div>
            <div class="sidebar-menu-label">MENU</div>
            <nav class="sidebar-links">${filteredMainLinks.map(link => createNavLink(link)).join('')}</nav>
            <div class="sidebar-footer">
                ...
                <button id="sidebar-logout-btn" class="sidebar-link logout-link desktop-only">
                    <div class="link-icon-wrapper"><svg class="icon"><use href="#power"/></svg></div>
    ...
    container.addEventListener('click', (e) => {
        e.stopPropagation();
        if (!document.body.classList.contains('sidebar-locked')) document.body.classList.add('sidebar-locked');
    });
    document.addEventListener('click', (e) => {
        if (document.body.classList.contains('sidebar-locked') && container && !container.contains(e.target)) {
    ...
    if (logoutBtn) {
        logoutBtn.addEventListener('click', async (e) => {
            ...
            const confirmed = await showConfirmationModal({ title: 'Log Out', message: 'Are you sure?', confirmText: 'Log Out', danger: true });
            if (confirmed) { await firebaseService.logout(); window.location.reload(); }
        });
    }
}
