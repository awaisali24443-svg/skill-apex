...
async function handleProgressUpdate(quizContext, score, missedConcepts, percentage) {
    ...
    if (quizContext.learningPathInfo) {
        ...
        if (score >= UNLOCK_SCORE && stepIndex < totalSteps) {
            await progressService.updateLearningPathProgress(pathId, stepIndex + 1);
            window.showToast("✅ Step Complete! Next step unlocked.", "success");
        } else {
             window.showToast("Complete this step with a better score to unlock the next one!", "error");
        }
    }
...
function renderResults() {
    ...
    if (!state) {
        resultsContainer.innerHTML = `
            <div class="results-summary">
                <h2 class="results-title">Error</h2><p>Could not load quiz results. The session may have expired.</p>
                <div class="results-actions"><a href="#home" class="btn btn-primary">Go to Dashboard</a></div>
            </div>`;
        return;
    }
    ...
    const passed = (quizContext.isLeveled || quizContext.learningPathInfo) ? score >= UNLOCK_SCORE : percentage >= 60;

    playSound(passed ? 'complete' : 'incorrect');

    let feedbackText = quizContext.isLeveled ? `<p>You needed ${UNLOCK_SCORE} correct answers to level up.</p>` : '';
    if (quizContext.learningPathInfo) {
        feedbackText = `<p>You needed ${UNLOCK_SCORE} correct answers to unlock the next step.</p>`;
    }
    ...
    resultsContainer.innerHTML = `
        ...
        <div class="results-feedback">${feedbackText}<p>${getFeedbackMessage(percentage)}</p></div>
